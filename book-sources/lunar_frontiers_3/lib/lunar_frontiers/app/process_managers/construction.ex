...
defmodule LunarFrontiers.App.ProcessManagers.Construction do
  ...
  # Process Routing (the "saga")

  # NOTE - all site IDs must be globally unique

  def interested?(%SiteSpawned{site_id: site_id}), do: {:start, site_id}

  def interested?(%ConstructionProgressed{
        site_id: site_id
      }),
      do: {:continue, site_id}

  def interested?(%ConstructionCompleted{
        site_id: site_id
      }),
      do: {:continue, site_id}

  def interested?(%BuildingSpawned{
        site_id: site_id
      }),
      do: {:stop, site_id}
  ...
  # Command Dispatch
  def handle(%__MODULE__{}, %ConstructionCompleted{} = event) do
    %ConstructionCompleted{
      site_type: site_type,
      site_id: site_id,
      location: location,
