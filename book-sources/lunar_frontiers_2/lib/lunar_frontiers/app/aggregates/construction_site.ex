...
defmodule LunarFrontiers.App.Aggregates.ConstructionSite do
  ...
  def execute(%ConstructionSite{} = _site,
    %SpawnSite{} = cmd) do
    %SpawnSite{
      site_id: id,
      site_type: typ,
  ...
  def execute(%ConstructionSite{} = site,
    ...
    |> Multi.new()
    |> Multi.execute(
      &progress_construction(&1, cmd.tick, cmd.advance_ticks))
  ...
  def apply(%ConstructionSite{} = site,
    %ConstructionProgressed{} = event) do
  ...
  def apply(%ConstructionSite{} = site,
    %ConstructionCompleted{} = event) do
    %ConstructionSite{
      site
      | completed: true,
