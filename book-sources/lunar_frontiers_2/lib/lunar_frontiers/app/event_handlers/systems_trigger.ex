...
defmodule LunarFrontiers.App.EventHandlers.SystemsTrigger do
  ...
  def handle(%GameloopAdvanced{tick: tick}, _metadata) do
    # On each tick determine which systems need to be
    # invoked/advanced. Some systems might not advance
    # on each tick. For example, you could have combat advance
    # every other tick (tick mod 2), movement advance every tick,
    # and resource generation every 3rd tick (tick mod 3).
